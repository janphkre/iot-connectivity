...
public class NFCSocketApduService extends HostApduService {
    ...
    static HashMap<Integer , WeakReference<Subscriber>> pending = new HashMap<>();
    ...
    public void handleWithSocket(SocketMessage message) {
        if(message instanceof SocketResponse) {
            WeakReference<Subscriber> pendingSocket = pending.get(((SocketResponse) message).getInReplyTo());
            if(pendingSocket == null) return;
            Subscriber subscriber = pendingSocket.get();
            if(subscriber == null) {
                pending.remove(((SocketResponse) message).getInReplyTo());
                return;
            }
            //TODO!
        }
