...
dbus_connection_send_with_reply_and_block(conn, msg, -1, &err);
        ...
        Eine Lösung, die Sockets in pharo verwaltet, wie es beispielhaft in \reflst{lst:bluetooth:pharo} zu sehen ist, würde auch wieder Mehraufwand gegenüber anderen Lösungen bedeuten, da andere Technologien nicht zwingend ebenfalls Sockets unterstützen und somit ein Mehraufwand durch abweichende Implementierungen entsteht, wenn mehrere Lösungen parallel genutzt werden.        
...
device->endpointAddr, buffer, sizeof(buffer), NULL, NULL, 0);
        \end{lstlisting}
        Jedes angeschlossene Android Gerät wird zunächst wie eine Datei über Methoden der Bibliothek geöffnet und eine Datenübertragung ermöglicht und durchgeführt (\reflst{lst:usb:server}). Das Senden und Empfangen von Daten ist dabei davon abhängig, welcher Befehl der Methode {\it libusb\_fill\_bulk\_transfer} übergeben wird. Je nach Methode werden Daten vom Buffer geladen oder in den gegebenen Buffer geschrieben.
    \subsubsection{Clientseite}
...
val outputStream = FileOutputStream(fd.fileDescriptor)
        ...
        Um langlebige Verbindungen wie USB abbilden zu können, muss wie in \reflst{lst:usb:socket} ein Stream definiert werden, welcher lediglich eine Fassade eines anderen Streams darstellt und lediglich sich selbst schließt, sodass der verpackte Stream nicht von der Anwendungsseite aus geschlossen werden kann.

        \begin{lstlisting}[frame=bt, label={lst:usb:socket}, language=Kotlin, caption=Socketverbindung für USB (Clientcode in Kotlin)]
